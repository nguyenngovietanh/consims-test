...
const model = require("../models/contractlinemember.js");
const modelmember = require("../models/member.js");
const modelcontractline = require("../models/contractline.js");
const express = require('express');
const router = express.Router();
const views = {
    list: { 'path': 'pages/contractlinemember', 'title': 'Submissions Members Management' },
    create: { 'path': 'pages/contractlinememberupdate', 'title': 'Create Submissions Members' },
    update: { 'path': 'pages/contractlinememberupdate', 'title': 'Update Submissions Members' },
};

router.get("/memberbycontractline", async (req, res) => {
    try {
        res.json(await model.getMemberByContractLineList({ req: req }));
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
// lấy danh sách data cho perform
router.get("/list4perform", async (req, res) => {
    try {
        res.json(await model.getList4Perform({ req: req }));
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

// router.get("/contractlinebymember", async (req, res) => {
//     try {
...
router.get("/", async (req, res) => {
    ...
    switch (viewname) {
        case 'pages/contractlinememberbycontractline':
            // Do something specific for contractlinememberbycontractline
            // res.locals.model.meta = modelmember.meta;
            break;
        // case 'pages/contractlinememberbymember':
        //     // Do something specific for contractlinememberbymember
...
router.post("/inherit", async (req, res) => {
    try {
        res.json(await model.postInherit({ req }));
    } catch (err) {
        res.status(500).json({ status: 0, message: err.message, errors: err.errors || err.message });
    }
});
