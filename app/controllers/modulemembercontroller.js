...
const model = require("../models/modulemember.js");
const modelmember = require("../models/member.js");
const modelmodule = require("../models/module.js");
const express = require('express');
const router = express.Router();
const views = {
    list: { 'path': 'pages/modulemember', 'title': 'Modules Members Management' },
    create: { 'path': 'pages/modulememberupdate', 'title': 'Create Modules Members' },
    update: { 'path': 'pages/modulememberupdate', 'title': 'Update Modules Members' },
};

router.get("/memberbymodule", async (req, res) => {
    try {
        res.json(await model.getMemberByModuleList({ req: req }));
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

router.get("/modulebymember", async (req, res) => {
    try {
        res.json(await model.getModuleByMemberList({ req: req }));
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
router.get("/", async (req, res) => {
    ...
    switch (viewname) {
        case 'pages/modulememberbymodule':
            // Do something specific for modulememberbymodule
            res.locals.model.meta = modelmember.meta;
            break;
        case 'pages/modulememberbymember':
            // Do something specific for modulememberbymember
