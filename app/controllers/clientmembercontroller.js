...
const model = require("../models/clientmember.js");
const modelmember = require("../models/member.js");
const modelclient = require("../models/client.js");
const express = require('express');
const router = express.Router();
const views = {
    list: { 'path': 'pages/clientmember', 'title': 'Clients Members Management' },
    create: { 'path': 'pages/clientmemberupdate', 'title': 'Create Clients Members' },
    update: { 'path': 'pages/clientmemberupdate', 'title': 'Update Clients Members' },
};

router.get("/memberbyclient", async (req, res) => {
    try {
        res.json(await model.getMemberByClientList({ req: req }));
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

router.get("/clientbymember", async (req, res) => {
    try {
...
router.get("/", async (req, res) => {
    ...
    switch (viewname) {
        case 'pages/clientmemberbyclient':
            // Do something specific for clientmemberbyclient
            res.locals.model.meta = modelmember.meta;
            break;
        case 'pages/clientmemberbymember':
            // Do something specific for clientmemberbymember
            res.locals.model.meta = modelclient.meta;
            break;
        default:
